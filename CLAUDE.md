...
src/
  ...
  torte/
    ...
    search/
      see.rs                    // static exchange evaluation (least-valuable-attacker swap, x-ray aware)
...
cargo run --release -- bench 8      # fixed positions at fixed depth; deterministic node total
cargo run --release -- tune bench/data/quiet-labeled.epd 1500   # refit eval weights -> params.rs
```
...
| `SeePruning`          | true    | qsearch skips captures the static exchange says lose material. −25% bench nodes. SPRT pending |
| `QsearchCheckEvasions`| true    | In check, qsearch drops the stand-pat and searches all evasions; mate at the horizon scores as mate. +14% bench nodes. SPRT pending |
| `TTDepthPreferred`    | true    | Keep the deeper TT entry on a collision, with per-search aging. −23% nodes over a game at 1 MB hash, −2% at 16 MB. SPRT pending |
| `LateMovePruning`     | true    | Skips quiet moves past 6/10/16 (by depth 1-3) outside check in a safe window. −2.4× nodes at bench depth 8, +1 ply per second. SPRT pending |
| `DeltaPruning`        | true    | In qsearch, skips a capture whose victim value + 200 cp can't reach alpha. ~13% fewer nodes. SPRT pending |
...
- **Search**: no MultiPV, no SMP. LMP and delta pruning landed 2026-09-16 (bench depth 8: 2.49M → 1.09M nodes, +1 ply per second) but are **unproven in games** — SPRT pending, along with `RookOpenFile`/`KingAttack`. (Killers, history, NMP, LMR, aspiration, futility and mid-search abort all landed — see the toggle table.)
- **TT**: single-entry buckets (no two-tier/bucketed replacement). Depth-preferred replacement with per-search aging landed 2026-09-16; the hash is still recomputed from scratch each node (no incremental Zobrist).
- **Quiescence**: skips promotion-only moves (no capture component). Check evasions and SEE pruning landed 2026-09-16; SEE is not yet used for *ordering* (captures are still MVV-LVA ordered, so losing captures are searched early in the main search).
- **Aspiration windows**: turning them on changes the reported depth-9 score on most test positions (kiwipete −88 → −144) while leaving the best move unchanged. Expected with TT bounds, but unexplained at that magnitude — see the 2026-09-16 log entry.
...
- **2026-09-16 — search: static exchange evaluation, SEE pruning in qsearch, and a root TT bound fix** — `see.rs` plays out the capture sequence on a square with the least valuable attacker each time, updating occupancy so x-rays behind a capturer join in. Two rules that matter: a **king may only take last** (capturing into a square the other side still attacks is illegal — without this the exchange reads as a king trade and SEE lies), and en passant is scored as a pawn capture. qsearch now skips captures with `see < 0` (toggle `SeePruning`, default on), never while evading check. `torte bench 8`: **1,244,878 → 935,999 nodes (−25%)**. Verified it isn't just pruning away accuracy: with `AspirationWindows` off, kiwipete depth 9 gives the **same score (−88) and same move with SEE on or off**, and SEE saves 13% of nodes there (586,744 → 507,568). Separately, the root stored `Bound::Exact` even when an aspiration search failed low or high, where the score is only a bound — now stored as Upper/Lower accordingly. That fix changes no current result (the root entry isn't probed for cutoffs), but an Exact score the search never established is a landmine for any future prober. **Open, unexplained:** `AspirationWindows` on vs off changes the depth-9 *score* on 3 of 4 test positions (kiwipete −144 vs −88) while the best move stays identical. Some drift is expected — with a TT, returned scores are bounds — but 56 cp is more than "slight" and nobody has shown it's benign. (5 new tests, 142 total.)
